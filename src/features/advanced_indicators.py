...
def calculate_advanced_features(
    df: pd.DataFrame, nifty50_df: Optional[pd.DataFrame] = None
) -> pd.DataFrame:
    """
    ...
    if nifty50_df is not None:
        ...
        df["correlation_to_nifty50_20d"] = (
            df["Daily_Return"].rolling(20).corr(df["Nifty50_Return"])
        )
        ...
        logger.warning(
            "NIFTY 50 data not provided, using default values for market context features"
        )
    ...
    df["hurst_exponent"] = (
        df["Close"].rolling(60).apply(lambda x: calculate_hurst(x.values), raw=False)
    )
