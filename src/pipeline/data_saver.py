...
def save_daily_results(
    date_str: str, llm_recs: List[Dict], ml_predictions: List[Dict]
) -> Path:
    """
        ...
        combined.append(
            {
                ...
                "llm_reason": llm_rec.get("reason_for_recommendation")
                if llm_rec
                else None,
    ...
    output = {
        "metadata": {
            ...
            "total_news_articles": len(
                set([r.get("news_id") for r in llm_recs if r.get("news_id")])
            ),
            "total_llm_recs": len(llm_recs),
            "total_validated": sum(1 for r in llm_recs if r.get("validated")),
            "total_ml_predictions": len(
                [p for p in ml_predictions if p["status"] == "SUCCESS"]
            ),
            "success_rate": len([p for p in ml_predictions if p["status"] == "SUCCESS"])
            / len(ml_predictions)
            if ml_predictions
            else 0,
        },
    ...
    logger.info(
        f"  ML predictions: {len([p for p in ml_predictions if p['status'] == 'SUCCESS'])}"
    )
